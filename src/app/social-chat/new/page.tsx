    ...
    <main className="min-h-screen bg-black text-white flex flex-col items-center justify-center p-6 bg-[radial-gradient(circle_at_center,_var(--tw-gradient-stops))] from-yellow-400/5 via-black to-black">
      <div className="w-full max-w-md space-y-12">
        <header className="text-center space-y-4">
            ...
            <p className="text-white/40 text-sm font-medium uppercase tracking-[0.2em]">What's on your mind?</p>
        </header>

        <form onSubmit={handleCreate} className="space-y-8">
            <div className="space-y-2">
                <label className="text-[10px] font-black uppercase tracking-[0.3em] text-white/30 ml-4">Room Topic</label>
                <input 
                    autoFocus
                    type="text" 
                    value={topic}
            ...
            <div className="space-y-4">
                <label className="text-[10px] font-black uppercase tracking-[0.3em] text-white/30 ml-4">Quick Vibe</label>
                <div className="flex flex-wrap gap-3">
                    {['Any', 'Chill', 'Deep', 'Gaming', 'Funny'].map(v => (
                        <button 
                            ...
                            type="button"
                            onClick={() => setVibe(v)}
                            className={`px-6 py-2.5 rounded-full text-[11px] font-black uppercase tracking-widest transition-all ${vibe === v ? 'bg-yellow-400 text-black' : 'bg-white/5 text-white/40 hover:bg-white/10'}`}
                        >
                            {v}
                        </button>
                    ))}
        ...
        <button 
          onClick={() => router.back()}
          className="w-full text-[10px] font-black uppercase tracking-[0.4em] text-white/20 hover:text-white/40 transition-colors"
        >
          Nevermind, go back
        </button>
      </div>
